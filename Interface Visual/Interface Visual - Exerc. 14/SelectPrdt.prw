...
/*/{Protheus.doc} User Function SelectPrdt
    Lista 04 - Interfaces Visuais | Exercício 14
    @type  Function
    @author André Lucas 
    @since 06/04/2023
    @version 0.1
...
User Function SelectPrdt()
    ...
    DEFINE MSDIALOG oDlg TITLE cTitle FROM 000, 000 TO 095, 330 FONT oFont := TFont():New('Arial',,-13,.T.) PIXEL 

    @ 014, 010 SAY 'Código do Produto: ' SIZE 85, 10 OF oDlg PIXEL 
    @ 014, 075 MSGET cPedVenda F3 "SB1"  SIZE 20, 10 OF oDlg PIXEL HASBUTTON

    @ 010, 125 BUTTON 'Ok'       SIZE 35,15 PIXEL OF oDlg ACTION (nOpcao := 1, oDlg:End())
    @ 027, 125 BUTTON 'Cancelar' SIZE 35,15 PIXEL OF oDlg ACTION (nOpcao := 2, oDlg:End())
