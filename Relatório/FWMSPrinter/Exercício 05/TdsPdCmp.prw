...
Static Function MontaRel(cAlias)
    ...
    Local cArquivo := 'Relatório Exer. 05 - Todos os Pedidos de Compra com Totalizador.pdf'
    ...
    Private oFt12SN := TFont():New('Arial', /*Compatibilidade*/, 12, /*Compatibilidade*/, .F., /*Compatibilidade*/, /*Compatibilidade*/, /*Compatibilidade*/, /*Compatibilidade*/, .F., .F.) //? .F. –> (Negrito, Sublinhado e Itálico)
    ...
    while (cAlias)->(!Eof())
        ...
        if cPedido != AllTrim((cAlias)->(C7_NUM))
            if nPag > 0 //! Apenas quando houver dados e for diferente de zero a contagem de páginas
